...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!isset($_POST['title']) || !isset($_POST['type']) || !isset($_POST['division_id'])) {
    ...
    try {
        ...
        if (move_uploaded_file($_FILES['document']['tmp_name'], $file_path)) {
            $query = "INSERT INTO documents (doc_id, title, type, division_id, file_path, file_name, file_size, mime_type, uploaded_by, uploaded_at, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), 1)";
        ...
        error_log("Document upload error: " . $exception->getMessage());
        http_response_code(500);
